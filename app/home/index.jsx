...
import hymns from "../../assets/hymns.js";
import { useRouter } from "expo-router";

export default function Home() {
  const router = useRouter();
  const hymnsTitle = hymns.map((d) => d.title);
        ...
        ListHeaderComponent={
          ...
          >
            <TextInput
              ...
              style={[{ width: "95%", height: 40, fontSize: 16 }]}
            />
            <View
        ...
        renderItem={({ item: title, index }) => {
          return (
            <TouchableOpacity
              onPress={() =>
                router.push({
                  pathname: `/hymns/${title.split(" ").join("-")}`,
                  params: { id: index },
                })
              }
            >
              <>
                ...
                {index + 1 != hymnsTitle.length && <Divider />}
              </>
            </TouchableOpacity>
          );
        }}
